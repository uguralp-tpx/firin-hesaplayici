...
if st.button("🔍 Hesapla"):
    ...
    # Gerekli Al artırımı hesaplaması
    adjusted_results = []
    for _, row in result_df.iterrows():
        if row["Ölçülen (%)"] > row["Max (%)"]:
            required_mass = (row["Hedef (kg)"] * 100) / row["Nominal (%)"]  # yeni toplam kütle ihtiyacı
            adjusted_furnace_mass = required_mass if required_mass > furnace_mass else furnace_mass
        else:
            adjusted_furnace_mass = furnace_mass
        hedef_kg = (row["Nominal (%)"] / 100) * adjusted_furnace_mass
        fark_kg = hedef_kg - row["Ölçülen (kg)"]
        durum = "✅" if row["Min (%)"] <= row["Ölçülen (%)"] <= row["Max (%)"] else "⚠️ Dışında"
        adjusted_results.append((row["Element"], row["Min (%)"], row["Nominal (%)"], row["Max (%)"], row["Ölçülen (%)"], hedef_kg, row["Ölçülen (kg)"], fark_kg, durum))

    result_df = pd.DataFrame(adjusted_results, columns=["Element", "Min (%)", "Nominal (%)", "Max (%)", "Ölçülen (%)", "Hedef (kg)", "Ölçülen (kg)", "Fark (kg)", "Durum"])
